...
const Navbar = () => {
    ...
    const [dismissedIds, setDismissedIds] = React.useState(() => {
        const saved = localStorage.getItem('prepnova_dismissed_ids');
    ...
    }, [user.achievements, user.hasCompletedOnboarding, dismissedIds]);

    const handleDismissAll = () => {
        const allIds = notifications.map(n => n.id);
        const newDismissed = [...new Set([...dismissedIds, ...allIds])];
        setDismissedIds(newDismissed);
        localStorage.setItem('prepnova_dismissed_ids', JSON.stringify(newDismissed));
            ...
            <div className="h-full flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-gold/10 border border-gold/20 rounded-lg">
                        ...
                        <span className="text-[10px] font-black text-gold">{user.streak || 0}</span>
                    </div>
                    ...
                    <div className="hidden md:flex flex-col items-end gap-0.5">
                        <div className="flex items-center gap-1">
                            ...
                            <span className="text-[10px] font-black text-white/70">LVL {user.level || 1}</span>
                        </div>
                                    ...
                                    >
                                        ...
                                        {notifications.length > 0 ? (
                                            ...
                                            <div className="py-8 text-center bg-white/[0.02] border border-dashed border-white/10 rounded-2xl">
                                                <p className="text-[10px] font-black text-text-muted uppercase tracking-widest">Sky is Clear</p>
                                                <p className="text-[8px] text-text-muted/50 mt-1 italic">No active alerts at this time</p>
                                            </div>
                                        )}

                                        <button
                                            onClick={handleDismissAll}
                                            className="w-full mt-6 py-3 rounded-xl bg-white/5 border border-white/10 text-[10px] font-black text-text-muted uppercase tracking-[0.2em] hover:bg-white/10 hover:text-white transition-all active:scale-95"
                                        >
                                            Dismiss All Scrolls
                                        </button>
                                    </motion.div>
